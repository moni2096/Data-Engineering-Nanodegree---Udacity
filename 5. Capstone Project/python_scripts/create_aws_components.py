...
# Reference: Parts of this code were taken from Infrastructure as a Code notebook in module 2 from Udacity Data Engineering Nanodegree
...
def create_redshift_cluster(redshift, roleArn, dwh_cluster_type, dwh_node_type, dwh_num_nodes, dwh_db,
...
def create_s3_bucket(s3_client, bucket_name, bucket_region):
    """
    Function to create S3 bucket
    """
    if not s3_client.Bucket(f"{bucket_name}") in s3_client.buckets.all():
...
def main():
    ...
    create_redshift_cluster(redshift, roleArn, DWH_CLUSTER_TYPE, DWH_NODE_TYPE, DWH_NUM_NODES, DWH_DB,
                            DWH_CLUSTER_IDENTIFIER, DWH_DB_USER, DWH_DB_PASSWORD)
    ...
    create_s3_bucket(s3, BUCKET_NAME, BUCKET_REGION)
